  ...
  return (
    <div className="fade-in" style={{ backgroundColor: '#080C14', minHeight: '100vh', paddingBottom: '36px', color: '#FFFFFF' }}>
      ...
      <div style={{ padding: '20px' }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: '12px', color: '#A2A2BA', margin: '2px 0 0 0' }}>
        ...
        <div style={{ fontSize: '11px', fontWeight: 800, color: '#A2A2BA', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '10px', paddingInlineStart: '4px' }}>
      ...
      <Modal isOpen={activeModal === 'preferences'} onClose={() => setActiveModal(null)} title={isAr ? 'تفضيلات البيانات' : 'Data Preferences'}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 14px', backgroundColor: '#151524', borderRadius: '12px', border: '1px solid #2C2C44' }}>
            <div>
              <div style={{ fontSize: '13.5px', fontWeight: 700, color: '#FFFFFF' }}>{isAr ? 'العروض المخصصة' : 'Personalized Offers'}</div>
              <div style={{ fontSize: '11px', color: '#A2A2BA', marginTop: '1px' }}>{isAr ? 'اقتراحات المكافآت والعروض' : 'Tailored rewards'}</div>
            </div>
            <input type="checkbox" checked={marketingConsent} onChange={(e) => setMarketingConsent(e.target.checked)} style={{ width: '18px', height: '18px', accentColor: '#7FE87F', cursor: 'pointer' }} />
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 14px', backgroundColor: '#151524', borderRadius: '12px', border: '1px solid #2C2C44' }}>
            <div>
              <div style={{ fontSize: '13.5px', fontWeight: 700, color: '#FFFFFF' }}>{isAr ? 'تحليلات المتاجر' : 'Merchant Analytics'}</div>
              <div style={{ fontSize: '11px', color: '#A2A2BA', marginTop: '1px' }}>{isAr ? 'مشاركة إحصاءات مجهولة المصدر' : 'Anonymized stats'}</div>
            </div>
            <input type="checkbox" checked={shareData} onChange={(e) => setShareData(e.target.checked)} style={{ width: '18px', height: '18px', accentColor: '#7FE87F', cursor: 'pointer' }} />
          </div>
      ...
      <Modal isOpen={activeModal === 'export'} onClose={() => setActiveModal(null)} title={isAr ? 'تصدير البيانات' : 'Export Data'}>
        {exportSuccess ? (
          <div style={{ textAlign: 'center', padding: '18px 0' }}>
            ...
            <p style={{ fontSize: '12px', color: '#A2A2BA', marginTop: '4px' }}>{isAr ? 'سيتم إرسال الملف إلى بريدك المسجل.' : 'Sent to your email shortly.'}</p>
          </div>
        ) : (
          <div style={{ textAlign: 'center', padding: '8px 0' }}>
            <p style={{ fontSize: '13px', color: '#A2A2BA', marginBottom: '18px', lineHeight: '1.4' }}>
      ...
      <Modal isOpen={activeModal === 'terms'} onClose={() => setActiveModal(null)} title={isAr ? 'شروط الخصوصية' : 'Privacy Statement'}>
        <div style={{ maxHeight: '260px', overflowY: 'auto', fontSize: '12.5px', color: '#A2A2BA', lineHeight: '1.5', display: 'flex', flexDirection: 'column', gap: '8px' }}>
